...
proptest! {
    ...
    fn property_06_platform_descriptors_exact_side_effect_free(
        ...
        let (engine, cli, revision) = match drift_case {
            ...
            1 => ("1.0.0-beta.11.rust.4", TARGET_CLI_VERSION, TARGET_REVISION),
        ...
        match (os, arch) {
            (Ok(os), Ok(arch)) => {
                ...
                let expected_name = format!("dagger_v1.0.0-beta.11.rust.4_{os_name}_{arch_name}.{extension}");
                ...
                prop_assert_eq!(descriptor.manifest_url().as_str(), "https://dl.dagger.io/dagger/releases/1.0.0-beta.11.rust.4/checksums.txt");
                prop_assert_eq!(descriptor.archive_url().as_str(), format!("https://dl.dagger.io/dagger/releases/1.0.0-beta.11.rust.4/{expected_name}"));
            }
...
fn six_release_descriptors_match_the_published_naming_policy() {
    ...
    for (os, arch, os_name, arch_name, format, member) in cases {
        ...
        let archive = format!("dagger_v1.0.0-beta.11.rust.4_{os_name}_{arch_name}.{extension}");
        ...
        assert_eq!(
            ...
            format!("https://dl.dagger.io/dagger/releases/1.0.0-beta.11.rust.4/{archive}")
        );
    }
}
...
fn explicit_discovery_expands_home_resolves_symlinks_and_ignores_irrelevant_cwd() {
    ...
    let resolved = native_fixture_path(
        "/opt/dagger-v1.0.0-beta.11.rust.4",
        r"C:\tools\dagger-v1.0.0-beta.11.rust.4.exe",
    );
