...
func moduleIdentity(ctx context.Context, source *dagger.ModuleSource, configFormat string) (scopedModuleIdentity, error) {
	...
	// Dagger's raw ModuleSource digest includes generated output and the VCS files
	// written after codegen. Hashing those products would make every fresh manifest
	// stale immediately. The semantic identity retains all implementation inputs
	// while the operation manifest authenticates the excluded generated bytes.
	digest, err := source.ContextDirectory().Directory(subpath).Filter(dagger.DirectoryFilterOpts{
		Exclude: []string{
	...
	return scopedModuleIdentity{
		...
		SourceDigest: metadata.DigestModuleSource(digest), ConfigFormat: configFormat,
	}, nil
}
