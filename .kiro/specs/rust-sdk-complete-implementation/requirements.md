...
**Ordinary build and verification — Rust-first**
...
- normal package checks produce the two public Rust crate packages;
- the Dagger build produces Rust SDK engine content and a complete engine container
  containing that content; and
- one isolated external Rust consumer installs the packaged crates and executes
  against the completed engine.
...
Release readiness uses ordinary Cargo checks and package validation, then the normal
Dagger build entry points to assemble Rust SDK engine content and a complete engine.
One isolated external Rust consumer verifies the packaged SDK against that engine;
behavioural authority from the Definitive_Go_SDK does not require its complete build
or test suite to run inside Rust verification.
...
- **Current version inconsistency:** `sdk/rust/Cargo.toml:6-8` declares SDK
  ...
  `1.0.0-beta.7`.
- **Ordinary Rust build path (post-baseline):**
  `.dagger/modules/rust-client-dev/main.go` packages `dagger-sdk-macros` and
  `dagger-sdk`, constructs Rust SDK engine content, assembles a complete engine, and
  verifies an isolated external consumer without publishing a crate.
- **Complete engine composition (post-baseline):**
  `.dagger/modules/engine-dev/main.go` accepts Rust SDK engine content and overlays it
  into the complete engine container.
...
| Security gates | Strong baseline exists but must cover every new component | Feature 9 | Locked, denied, audited, checksum-verifying, and secret-safe checks pass |
...
### Requirement 9.1: Coherent Packages, Engine, and Version

**User Story:** As a Rust SDK consumer, I want coherent crate packages and a matching
complete engine, so that each release artifact represents one compatible source
identity.

#### Acceptance Criteria

1. WHEN release architecture is finalized, THE workspace SHALL classify every crate as
   public and packageable or private and non-publishable.
2. WHEN release artifacts are built, THE ordinary Rust build SHALL package exactly the
   two public crates, `dagger-sdk-macros` and `dagger-sdk`.
3. WHEN either public crate is packaged, THE packaged crate graph SHALL compile from a
   clean source tree without relying on a private workspace crate.
...
5. WHEN version-bearing files disagree, THE release pipeline SHALL fail before
   artifact creation.
6. WHEN the Dagger release build runs, THE build SHALL produce Rust SDK engine content
   and a complete engine container containing that content.
7. WHEN package validation runs, THE build SHALL package and compile both public crates
   using normal Cargo commands and the committed lockfile.
8. WHEN downloadable release artifacts are prepared, THE release path SHALL produce a
   checksum file covering both crate packages and the complete engine archive.
9. WHEN the declared MSRV or edition changes, THE release notes SHALL state the
...
7. WHEN the release candidate is evaluated by an external Rust consumer, THE
   acceptance suite SHALL install the packaged crates outside the repository workspace
   and execute a representative query against the completed engine.
...
### Requirement 9.3: Verification, Safety, and Publication Boundary

**User Story:** As a release reviewer, I want a small explicit release boundary, so
that ordinary SDK validation preserves user safety without introducing a parallel
distribution system.
...
4. WHEN dependency policy is evaluated, THE verification SHALL reject active
...
6. WHEN the SDK downloads a CLI, THE SDK SHALL verify the downloaded archive against
   its expected checksum before execution.
7. WHEN tests, errors, traces, or snapshots are produced, THE SDK and verification
   tooling SHALL keep configured secrets and session credentials out of diagnostics.
8. WHEN an engine session starts, THE SDK SHALL reject incompatible engine version or
   revision metadata with a typed diagnostic.
9. THE maintained release path SHALL NOT add TUF metadata, GitHub Actions workflows,
   crates.io publication, or Dagger CLI surface area.
10. THE maintained specifications, code, and documentation SHALL NOT name products or
    platforms belonging to external consumers.
