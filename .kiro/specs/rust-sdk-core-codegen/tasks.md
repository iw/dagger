...
- [x] 1. Establish the exact Feature 4 scope and Rust code-generation foundations
  - [x] 1.2 Reshape crate dependencies around a pure code generator
    ...
    - Move the raw introspection wire model into `dagger-codegen` without exposing it
      ...
      projection, catalog, and renderer modules with no filesystem, process, network, or
      engine I/O.
    - Remove `dagger-bootstrap`'s dependency on `dagger-sdk` and retain `proptest`,
      `trybuild`, and `tempfile` as the workspace-standard verification/orchestration
      tools.
...
- [x] 2. Checkpoint: scope, dependencies, and test foundations are green
  - Run formatting, locked checking, codegen unit and property tests, clippy,
    rustdoc, and cargo-deny; require `dagger-codegen` to compile without `dagger-sdk`
    or an I/O/runtime dependency.
...
- [x] 3. Implement exact-target input and the canonical schema compiler
  - [x] 3.1 Add target identity, bounded schema input, and digest verification
    - Decode `CodegenTarget` exclusively from `codegen/target.json`, including
...
- [x] 4. Checkpoint: exact-target canonical schema compilation is green
  - Run formatting, locked codegen tests, schema fixtures, Properties 3-5,
...
- [x] 5. Implement recursive type, naming, scalar, and directive projection
  ...
  - [x] 5.4 Build the semantic projection catalog and fingerprints
    - Produce exact binding keys and implementation fingerprints from canonical wire
      ...
      strategies, and symbol paths; exclude formatting from semantic
    ...
    - _Requirements: 4.15, 10.1-10.3, 10.7_
...
- [x] 14. Implement deterministic check/update orchestration and confined publication
  - [x] 14.1 Add the typed bootstrap generation command
    ...
    - Read only regular no-follow target/schema/manifest inputs, return
...
- [x] 15. Checkpoint: repository generation is deterministic and failure-atomic
  - Run formatting, locked bootstrap/codegen tests, Properties 23-27,
...
- [x] 16. Close the exhaustive binding manifest
  - [x] 16.2 Assemble the generated binding manifest as an exact join
    - Join the target descriptor, projection catalog, and formatted artifacts into
      canonical JSON (today `codegen/generated.json`).
    - Require exactly one fingerprint-matching record for every active schema
      coordinate, explicit binding kind/symbol-or-policy/implementation fingerprint,
      and reviewed decision IDs for every materially different Rust public shape; the
      manifest is an inventory, not a status assertion.
    - _Requirements: 9.1-9.3, 10.1, 10.2, 10.3, 10.19, 10.20_
  - [x] 16.4 Property test: Property 2 — binding closure is a capability bijection
    - Generate at least 256 catalogs and decision combinations around the exact
      retained target; compare manifest acceptance to an independent set/bijection
      model.
    - Test identifier: `property_02_binding_closure_capability_bijection`.
    - _Requirements: 10.1, 10.2, 10.3, 10.19, 10.20_
...
- [x] 17. Wire repository generation and exact-target core conformance
  - [x] 17.1 Replace the live-schema/cargo-fix Dagger generation path
    - Change `.dagger/modules/rust-client-dev` so `WithGeneratedClient` invokes checked-input
    ...
    - Add `GeneratedClientCheck` for check mode, positive/negative compile suites,
      exhaustive query projection, warning-denied rustdoc, and owned-manifest
  ...
  - [x] 17.2 Add focused exact-target generated-client conformance
    ...
    - Record results deterministically and only for behaviours actually observed
  ...
  - [x] 17.3 Property test: Property 29 — exact-target conformance spans every generated category
    - Generate at least 256 conformance records with target, command, category,
      operation, and result mutations; compare acceptance to a reference category
      matrix and exhaust the finite live Exact_Target category set.
...
- [x] 18. Checkpoint: manifest closure and exact-target conformance are green
  - Run formatting, locked workspace tests, Properties 2 and 29, generated check,
    all compile/projection suites, exact-target `CoreConformance`, rustdoc, clippy,
    and cargo-deny; require the conformance report to name the exact target and only
    its observed scope.
...
- [x] 19. Stabilize documentation, public compatibility, and committed outputs
  - [x] 19.1 Document the durable generated-client and maintenance contracts
    - Add `//!` ownership/invariant documentation to handwritten generator/bootstrap/
      runtime modules and `///` guarantees, caller assumptions, failure,
    ...
    - Add a maintainer guide for target refresh, mapping review, check/update, localized
      generated diffs, manifest interpretation, rollback recovery, exact-target
  ...
  - [x] 19.3 Publish the first complete generated artifact set into the worktree
    ...
    - _Requirements: 9.1-9.13, 10.1-10.20_
  - [x] 19.4 Fence dependency and generated-surface security policy
    ...
    - Keep generated/runtime code under `unsafe_code = "deny"`; add regression tests
      ...
      failure before the final gate is accepted.
...
- [x] 20. Final checkpoint: Feature 4 is releasable
  ...
  - Require all retained property identifiers, all target-wide finite inventories,
    compile pass/fail suites, 720-field/611-argument projection coverage, exact-target
    conformance, binding-manifest bijection, public API/doc fences, and a clean
    generated-output check to pass.
...
- The generated binding manifest is an exact inventory join, not a status assertion.
