...
Feature 2 defines the public contract consumed by Feature 3, while Feature 3 owns
session-source precedence, CLI discovery and download, HTTP authentication,
OpenTelemetry propagation, transport retry behaviour, and the detailed transport
error taxonomy. Feature 4 owns complete schema-derived binding generation; Feature 9
owns release migration material and the final stable publication gate; Feature 10
owns the runtime-integrity extensions layered onto this contract, including
cooperative module cancellation and runtime-thread scheduling safety. Feature 2 may
change generated handle storage through the generator when lifecycle ownership
requires it, but it does not add or alter schema mappings.

This specification was reinstated after the removal of the deprecated
completeness/evidence delivery machinery: the capability requirements below are
maintained, the retired ledger apparatus is not. Repository citations in the
evidence section are pinned to the historical Target_Revision and describe the
baseline that motivated this feature; the delivered implementation lives in
...
observability, every generated schema binding, or the full release matrix is
complete. Where a Feature 2 capability depends on Feature 3 behaviour, its
verification is complete only when the required cross-feature tests exist.
...
## Lifecycle Policy Statements

These stable policy statements govern the lifecycle contract. Their identifiers are
...
18. THE Engine_Connection abstraction SHALL document ownership transfer and close
    semantics.
19. WHEN a Client or query-construction handle is formatted with `Debug`, THE output
    SHALL render only the shared session's allocation identity.
20. WHEN the Client_Config builder is formatted with `Debug`, THE output SHALL render
    presence and keys only, exactly as the built Client_Config does.
